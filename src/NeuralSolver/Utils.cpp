...
torch::Tensor nvtt_bc3(torch::Tensor src/*[c,d,h,w]*/)
{
	...
	nvtt::Format compform = nvtt::Format::Format_BC3;
	...
	if (inputImage.num_channels != 4)
		fprintf(stderr, "Error: nvtt_bc3 inputImage.num_channels != 4\n");
	...
	if (!decimage.save("image/nvtt_bc3_test.png"))
...
void nvtt_bc7(UMBD::FTex3D& Tex)
{
	nvtt::Format compform = nvtt::Format::Format_BC7;
	nvtt::RefImage inputImage;
	inputImage.width = (int)Tex.GetGrid().Width;
	inputImage.height = (int)Tex.GetGrid().Height;
	inputImage.depth = (int)Tex.GetGrid().Depth;
	inputImage.num_channels = (int)Tex.GetNumChannel();
	if (inputImage.num_channels != 4)
		fprintf(stderr, "Error: nvtt_bc7 inputImage.num_channels != 4\n");
	inputImage.channel_interleave = false;
	std::vector<unsigned char> pixels;
	pixels.resize(Tex.GetNumElements());
	for (const auto& Point : Tex.GetGrid())
	{
		for (uint64_t C = 0;C < Tex.GetNumChannel();C++)
		{
			pixels[C * Tex.GetGrid().GetOuterVolume() + Tex.GetGrid().GetIndex(Point)] =
				UMBD::ElementFloatClampToUint8(Tex.GetFloat(Point, C));
		}
	}
	inputImage.data = pixels.data();
	std::streamsize outputSizeBytes{};
	{
		nvtt::Context            context;
		nvtt::CompressionOptions options;
	...
	for (const auto& Point : Tex.GetGrid())
	{
		...
		{
			Tex.SetFloat(Point, C, decimage.data()[Tex.GetGrid().GetOuterVolume() * C + Tex.GetGrid().GetIndex(Point)]);
		}
	}
}
