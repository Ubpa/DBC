...
BC7::BC7(at::DeviceType device, int refinecount, int epoch, float lr, bool* use_mode, QuantizeMode quantizeMode, OptimizeMode optimizeMode, Mode7Type mode7Type) : Compressor(device, refinecount, epoch, lr, quantizeMode, optimizeMode) {
	_code456.resize(8);
	...
	if (_use_mode[7] == true)
	{
		_bc7_partition2 = _bc7_partition2.reshape({ 64,16 }).to(device);
		for (int i = 0; i < 64; ++i)
		{
...
void BC7::encode(float roundc, double tau, double noisy)
{
	...
	if (_use_mode[7] == true)
	{
		if (_init_mode7_weight)
		{
			for (int partition = 0; partition < 64; ++partition)
			{
				Tensor* c0 = _code7[partition][0];
				Tensor* c1 = _code7[partition][1];
				Tensor* mask = _code7[partition][2];
			...
			for (int partition = 0; partition < 64; ++partition)
			{
				...
				Tensor* c1 = _code7[partition][1];
				Tensor* mask = _code7[partition][2];
				Tensor dec0 = subset_decode(_src, c0[0], c1[0], mask[0], roundc, 3, 31);
				Tensor dec1 = subset_decode(_src, c0[1], c1[1], mask[1], roundc, 3, 31);
				Tensor dest7 = torch::cat({ dec0,dec1 }, 2);
			...
			//_mode7_learned_weight = _mode7_learned_weight.scatter_(1, index.unsqueeze(1), 1.0);
			_mode7_learned_weight = 1 / error.detach();
			_mode7_learned_weight.set_requires_grad(false);
			auto index = std::get<1>(error.min(0, false)); //[n]
		}
		...
		_MoPIndices = MoPSelect(_mode7_learned_weight, noisy, _Ns, _Nr);
		...
		Tensor* c0 = _code7[0][0];
		Tensor* c1 = _code7[0][1];
		Tensor* mask = _code7[0][2];
...
Tensor BC7::decode(const std::vector<Tensor>& code)
{
	Tensor dest;
	return dest;
}

Tensor BC7::qdecode(const std::vector<Tensor>& code, float roundc, double tau, double noisy)
{
	std::vector<Tensor> dests;
	if (_use_mode[4] == true || _use_mode[5] == true)
	{
		...
		for (int i : qidxs)
		{
			Tensor qAlphamask = QuantizeAlphaMask(Alphamask, Alphaqmax[i], Alphamaskqmax[i], roundc); //[rot,n,b*b]
			Tensor qRGB = subset_decode(srcRGB, c0, c1, mask, roundc, maskqmax[i], colorqmax[i]);
	...
	if (_use_mode[6] == true)
	{
		...
		dests.push_back(subset_decode(_src, c0, c1, mask, roundc, 15, 255 /*7+1*/));
	}

	int mode7beginindex = dests.size();
	if (_use_mode[7] == true)
	{
		int colorqmax = 63; //5+1 //31;
		Tensor* c0 = _code7[0][0];
		Tensor* c1 = _code7[0][1];
		Tensor* mask = _code7[0][2];
		Tensor dec0 = subset_decode(_src, c0[0], c1[0], mask[0], roundc, 3, colorqmax);//[n,b*b,c]
		Tensor dec1 = subset_decode(_src, c0[1], c1[1], mask[1], roundc, 3, colorqmax);//[n,b*b,c]
	...
	{
		_mode7_learned_weight.index_put_({
				_MoPIndices,
	...
	if (_modeweight == nullptr)
	{
		...
		if (_mode7Type == Mode7Type::BruteForce
			...
			tau = 0, noisy = 0;
		*_modeweight = AutoMax(-error, tau, noisy, true, 0);
	}
...
Tensor BC7::forward(const Tensor& src)
{
	_src = src.clone().detach().to(_device).requires_grad_(true);
	float roundc = 0;
	encode(roundc);
	_dest = qdecode(getcode(), roundc);
	return _dest;
}
Tensor BC7::backward(const Tensor& gradinput)
{
